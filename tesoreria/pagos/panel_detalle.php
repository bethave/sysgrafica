<?php
	...
	$detalles = pg_query($conexion, "SELECT * FROM v_pagos_reporte WHERE pago_cod = $id_oc;");
	...
	$pedidos = pg_query($conexion, "SELECT * FROM v_venta_factura WHERE fac_estado = 'ORDENADO';");
 	...
 	$factrep = pg_fetch_all($factura);
        $cuenta = pg_query($conexion, "SELECT * FROM v_cuenta_pagar where estado = 'PENDIENTE';");
 	...
 	$ban = pg_fetch_all($banco);
        $ped_det2 = null;
        foreach($detalle as $pd){
            $detalle[$pd['fac_cod']][$pd['id_item']]['fac_cod'] = $pd['fac_cod'];
            $detalle[$pd['fac_cod']][$pd['id_item']]['id_item'] = $pd['id_item'];
            $detalle[$pd['fac_cod']][$pd['id_item']]['item_descrip'] = $pd['item_descrip'];
            $detalle[$pd['fac_cod']][$pd['id_item']]['precio'] = $pd['precio'];
            $detalle[$pd['fac_cod']][$pd['id_item']]['cantidad'] = $pd['cantidad'];
            $detalle[$pd['fac_cod']][$pd['id_item']]['iva'] = $pd['iva'];
            $detalle[$pd['fac_cod']][$pd['id_item']]['monto'] = $pd['monto'];
        }
?>
<div class="row">
    <div class="card">
        ...
        <div class="card-body">
            ...
            <?php }else{?>
                <?php foreach($ped as $p){?>
                    <div class="row" id="contenedor<?php echo $p['pago_cod'];?>">
                        <div class="card">
                            <div class="card-header">
                                <input type="checkbox" style="width: 25px; height: 25px;" class="check_pedido" value="<?php echo $p['pago_cod'];?>">&nbsp;&nbsp;
                                <b>Nro Factura: </b><?php echo $p['pago_cod'];?>&nbsp;&nbsp;
                                <b>Sucursal:</b><?php echo $p['suc_nombre'];?>&nbsp;&nbsp;
                                <b>Fecha:</b><?php echo $p['pago_fecha'];?>&nbsp;&nbsp;
                                <a class="btn btn-info" data-toggle="collapse" data-parent="#contenedor<?php echo $p['pago_cod'];?>" href="#pedido<?php echo $p['pago_cod'];?>">
                                    Ver Detalles
                                </a>
                            </div>
                            <div class="card-body panel-collapse collapse in" id="pedido<?php echo $p['pago_cod'];?>">
...
<div class="row">
    <button type="button" class="btn btn-primary" onclick="grabar_pedidos(<?php echo $cab[0]['pago_cod'];?>);">GRABAR FACTURA</button>
</div>-->
<div class="row">
    <div class="card col-sm-12">
        <div class="card-header p-0">
        ...
        <div class="card-body">
            <?php if(empty($cbr)){ ?>
                <label style="color: red">NO HAY CUENTAS A PAGAR DISPONIBLES</label>
            <?php }else{ ?>
                <br><b>SELECCIONE EL CUENTA A PAGAR</b><br>
                <select id="id_cuenta" class="form-control col-sm-6 select2">
                    <?php foreach($cbr as $c){ ?>
                    <option value="<?php echo $c['cpag_cod'];?>">Código: <?php echo $c['cpag_cod'];?> - Cod_factura:  <?php echo $c['cod_doc'];?> - Estado: <?php echo $c['estado'];?> - Monto: <?php echo $c['cpag_monto'];?></option>
                    <?php } ?>
                </select><br>
                ...
                <dl></dl>
                <button type="button" class="btn btn-block btn-success col-sm-4" onclick="agregar_detalle(<?php echo $cab[0]['pago_cod'];?>);">
...
<table id="tabla_panel_pedido" class="table table-bordered table-striped">
	...
	<tbody>
		<?php foreach($det as $d){?>
                    <tr>
			<td><?php echo $d['pago_cod'];?></td>
                        <td><?php echo $d['cuen_nro'];?></td>
			<td>Código: <?php echo $d['cod_doc'];?> - Cuotas: <?php echo $d['tipo_doc'];?></td>
			...
			<td>
                            <button title="Editar" type="button" class="btn btn-warning" onclick="editar_detalle(<?php echo $d['pago_cod'];?>,<?php echo $d['cuen_nro'];?>,<?php echo $d['fc_id'];?>,<?php echo $d['ban_cod'];?>,<?php echo $d['importe'];?>)">
                                <i class="fas fa-pen"></i>
                            </button>
                            <button title="Eliminar" type="button" class="btn btn-danger" onclick="eliminar_detalle(<?php echo $d['pago_cod'];?>,<?php echo $d['cuen_nro'];?>,<?php echo $d['fc_id'];?>,<?php echo $d['ban_cod'];?>)">

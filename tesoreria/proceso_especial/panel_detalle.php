<?php
	...
	$detalles = pg_query($conexion, "SELECT * FROM v_venta_remision_detalle WHERE rem_cod = $id_oc;");
 	$det = pg_fetch_all($detalles);
	$items = pg_query($conexion, "SELECT * FROM items WHERE id_item NOT IN (SELECT id_item FROM venta_remision_detalle WHERE rem_cod = $id_oc);");
...
<div class="row">
    <div class="card">
        ...
        <div class="card-body">
            ...
            <b>Sucursal Salida: </b><?php echo $cab[0]['suc_salida'];?><br>
            ...
            <b>Sucursal Llegada: </b><?php echo $cab[0]['suc_llegada'];?><br>
        </div>
    </div>
</div>
...
<div class="row">
    <div class="card col-sm-12">
        <div class="card-header p-0">
            <b>SELECCIONE ORDEN O CHEQUE</b>
        </div>
        <div class="card-body">
            <?php if(empty($ped)){?>
            <label style="color:red">NO HAY CHEQUE U ORDEN DISPONIBLE</label>
            <?php }else{?>
...
<?php } ?>
<div class="row">
    <button type="button" class="btn btn-primary" onclick="grabar_pedidos(<?php echo $cab[0]['rem_cod'];?>);">GRABAR CHEQUE U ORDEN</button>
</div>
<!--<div class="row">
    <div class="card col-sm-12">
        <div class="card-header p-0">
...
<div class="row">
<?php if (empty($det)) { ?>
<label style="color: red">NO SE ENCUENTRAN DETALLES PARA ESTA REMISIÓN</label>
<?php }else{ ?>

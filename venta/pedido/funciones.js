...
function anular_cabecera(id_ped){
    $("#btn-modal-anular").click();
    $("#id_ped_anular").val(id_ped);
}
function grabar_anular(){
    var id_ped = $("#id_ped_anular").val();
    $.ajax({
    type: "POST", url: "grabar.php",
      data: {
          id_ped : id_ped, 
...
function imprimir(id_ped){
    /*var id_ped = $("#id_ped_confirmar").val(id_ped);
    var cli_id = $("#descripcion").val(cli_id);
    var id_item = $("#id_item").val(id_item);
    window.open ("../../tcpdf/examples/example_pedido_venta.php?codigo="+id_ped);*/
    var getMyFrame = document.getElementById(id_ped);
    getMyFrame.focus();

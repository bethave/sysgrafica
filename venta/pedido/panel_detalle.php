<?php
	...
	$detalles = pg_query($conexion, "SELECT * FROM v_venta_pedido_detalle WHERE id_ped = $id_ped;");
 	$det = pg_fetch_all($detalles);
	$items = pg_query($conexion, "SELECT * FROM items WHERE id_item NOT IN (SELECT id_item FROM venta_pedido_detalle WHERE id_ped = $id_ped);");

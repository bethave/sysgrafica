...
function agregar(){
  ...
  $("#gentilicio_agregar").select();
}
function editar(codigo, descripcion, gentilicio){
  $("#descripcion_editar").select();
  $("#gentilicio_editar").select();
  $("#codigo_editar").val(codigo);
  $("#descripcion_editar").val(descripcion);
  $("#gentilicio_editar").val(gentilicio);
  $("#descripcion_anterior_editar").val(descripcion);
  $("#btn-modal-editar").click();
  $("#gentilicio_anterior_editar").val(gentilicio);
  $("#btn-modal-editar").click();
}
...
function grabar(operacion){
  ...
  if(operacion=='1'){ //AGREGAR
   ...
   var gentilicio = $("#gentilicio_agregar").val().trim();
    if(descripcion!=""){
  ...
  if(operacion=='2'){
    ...
    gentilicio = $("#gentilicio_editar").val().trim();
    if((descripcion!="") && (descripcion != $("#descripcion_anterior_editar").val()) && (gentilicio != $("#gentilicio_anterior_editar").val())){
      $("#descripcion_editar").removeClass("is-warning");
      $("#gentilicio_editar").removeClass("is-warning");
      $("#btn_cerrar_editar").click();
      $("#descripcion_editar").val("");
      $("#gentilicio_editar").val("");
      $("#descripcion_anterior_editar").val("");
      $("#gentilicio_anterior_editar").val("");
      grabar = 'SI';
    }else{
  ...
  if(grabar == 'SI'){
    $.ajax({
      type: "POST", url: "grabar.php", data: {codigo:codigo, descripcion:descripcion, gentilicio:gentilicio, operacion:operacion}

...
<!--                 <input class="form-control" type="text" id="pais_agregar" placeholder="Pais"><br>-->
                  ...
                  <select id="pais_agregar" class="form-control select2">
                      <?php $conexion = conexion::conectar();
                        $r_pais = pg_query($conexion, "SELECT * FROM v_personas WHERE estado = 'ACTIVO';");
                        $pais = pg_fetch_all($r_pais);
                      ?>
                      <?php foreach($pais as $c){ ?>
                      <option value="<?php echo $c['id_per']; ?>"><?php echo $c['persona']; ?> - <?php echo $c['per_ruc']; ?></option>
                      <?php } ?>
                  </select>
                </div>
...
-->                  <input type="hidden" id="codigo_editar" value="">
                  <label>Seleccione una Persona</label>
                  <select id="pais_anterior_editar" class="form-control select2">
                      <?php $conexion = conexion::conectar();
                        $r_pais = pg_query($conexion, "SELECT * FROM v_personas WHERE estado = 'ACTIVO';");
                        $pais = pg_fetch_all($r_pais);
                      ?>

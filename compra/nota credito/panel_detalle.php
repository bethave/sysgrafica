<?php
	...
	$detalles = pg_query($conexion, "SELECT * FROM v_ncd_compra_detalle WHERE cred_cod = $id_oc;");
 	$det = pg_fetch_all($detalles);
	$items = pg_query($conexion, "SELECT * FROM items WHERE id_item NOT IN (SELECT id_item FROM detalle_ncd_compra WHERE cred_cod = $id_oc);");
 	$ite = pg_fetch_all($items);
	$pedidos = pg_query($conexion, "SELECT * FROM v_factura_compra WHERE fac_estado = 'CONFIRMADO' or fac_estado = 'CON NOTA DEBITO' or fac_estado = 'CON NOTA CREDITO';");
 	$ped = pg_fetch_all($pedidos);
        $pedidos_detalles = pg_query($conexion, "SELECT * FROM v_factura_compra_detalle WHERE fac_cod IN (SELECT fac_cod FROM factura_compra WHERE fac_estado = 'CONFIRMADO' or fac_estado = 'CON NOTA DEBITO' or fac_estado = 'CON NOTA CREDITO');");
...
<div class="row">
    <div class="card">
        ...
        <div class="card-body">
            ...
            <b>Sucursal: </b><?php echo $_SESSION['suc_nombre'];?><br>
	...
	<tbody>
		<?php foreach($det as $d){?>
			...
			<td><?php echo $d['iva'];?></td>
                        <td><?php echo $d['ncd_precio']*$d['ncd_cantidad'];?></td>
                        <td>
                            <button title="Editar" type="button" class="btn btn-warning" onclick="editar_detalle(<?php echo $d['cred_cod'];?>,<?php echo $d['id_item'];?>,<?php echo $d['ncd_cantidad'];?>)">

<?php
	...
	$detalles = pg_query($conexion, "SELECT * FROM v_presupuesto_compra_detalle WHERE pre_cod = $id_oc;");
 	$det = pg_fetch_all($detalles);
	$items = pg_query($conexion, "SELECT * FROM items WHERE id_item NOT IN (SELECT id_item FROM detalle_presu_compra WHERE pre_cod = $id_oc);");
...
<?php } ?>
<div class="row">
    <button type="button" class="btn btn-primary" onclick="grabar_pedidos(<?php echo $cab[0]['pre_cod'];?>);">GRABAR PEDIDOS</button>
</div>
<div class="row">
    <div class="card col-sm-12">
        <div class="card-header p-0">

<?php
	...
	$resultado = pg_query($conexion, "SELECT * FROM v_compra_pedido;");
		...
		<?php foreach($datos as $d){?>
			...
			<td>
                            ...
                            <?php if($d['estado']=='CONFIRMADO'){ ?>
                                <button title="imprimir" type="submit" class="btn btn-outline-warning" onclick="imprimir(<?php echo $d['id_ped']; ?>)">PRINT
                                <i class="fas fa-check"></i>
                            </button>
                            <?php } ?>
                        </td>
